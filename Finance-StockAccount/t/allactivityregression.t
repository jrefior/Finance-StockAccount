...

{
    ...
    print $sa->maxCashInvested(), "\n";
    # should be > 15,989 ?
    ok($sa->maxCashInvested() =~ /^15277\./, 'Got expected max cash invested.');
    ok($sa->profitOverCostBasis() =~ /^0\.0964/, 'Got expected profit over outlays.');
    ok($sa->profitOverOutlays() =~ /^0\.0728/, 'Got expected profit over outlays.');
    ok($sa->profitOverMaxCashInvested() =~ /^0\.6519/, 'Got expected profit over max cash invested.');
    ok($sa->profitOverYears() =~ /^4259\./, 'Got expected profit over years.');
    ok($sa->profitOverMaxCashInvestedOverYears() =~ /^0\.27/, 'Got expected profit over max cash invested over years.');
    ...
    # print $quarterlyStats->[4]{maxCashInvested}, "\n";
    ok($quarterlyStats->[4]{maxCashInvested} =~ /^285\./, 'Got expected maxCashInvested for the fifth quarterly stats calculation.');
