...

{
    ...
    is($set->totalOutlays(), 4461, 'Cost (totalOutlays) as expected.');
    is($set->totalCostBasis(), 3673, 'Cost basis for realizations as expected.');
    is($set->totalRevenues(), 4040, 'Benefit (revenue) as expected.');
    is($set->profit(), 367, 'Profit as expected.');
    ok($set->profitOverOutlays() =~ /^0\.0822/, 'Profit over outlays as expected.');
    ok($set->profitOverCostBasis() =~ /^0\.0999/, 'Profit over outlays as expected.');
